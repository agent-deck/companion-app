//! AgentDeck Daemon - Background process that owns the HID device
//!
//! Provides WebSocket (exclusive) and HTTP REST (shared) APIs for
//! controlling the AgentDeck macropad.
...
use agentdeck_protocol::{AppControlAction, DEFAULT_DAEMON_ADDR};
...
#[derive(Parser)]
#[command(name = "agentdeck-daemon", about = "AgentDeck background daemon")]
struct Cli {
    /// Listen address
...
fn main() {
    ...
    info!("Starting AgentDeck daemon on {}", cli.listen);
...
fn install_launchd(listen: &str) {
    #[cfg(target_os = "macos")]
    {
        ...
        let plist_path = format!("{}/com.agentdeck.daemon.plist", plist_dir);
...
<dict>
    <key>Label</key>
    <string>com.agentdeck.daemon</string>
    ...
    <string>{home}/Library/Logs/agentdeck-daemon.log</string>
...
fn uninstall_launchd() {
    ...
    {
        let home = std::env::var("HOME").expect("HOME not set");
        let plist_path = format!("{}/Library/LaunchAgents/com.agentdeck.daemon.plist", home);
