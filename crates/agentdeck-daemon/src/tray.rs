...
impl DaemonTrayManager {
    pub fn new() -> Result<(Self, std::sync::mpsc::Receiver<DaemonTrayAction>)> {
        ...
        let toggle_item = MenuItem::new("Show Agent Deck", false, None);
        ...
        let tray = TrayIconBuilder::new()
            ...
            .with_tooltip("Agent Deck Daemon - Disconnected")
    ...
    pub fn set_device_status(&mut self, presence: DevicePresence, device_name: Option<&str>) {
        ...
        let tooltip = match presence {
            DevicePresence::Active => {
                format!("Agent Deck Daemon - {}", device_name.unwrap_or("Active"))
            }
            DevicePresence::Available => {
                format!("Agent Deck Daemon - {} (idle)", device_name.unwrap_or("Available"))
            }
            DevicePresence::None => "Agent Deck Daemon - No device".to_string(),
        };
    ...
    pub fn set_app_connected(&mut self, connected: bool) {
        ...
        if connected {
            self.toggle_item.set_text("Show Agent Deck");
        } else {
            self.toggle_item.set_text("Show Agent Deck");
        }
    }
