...
//! Core Deck Companion App - Entry Point
//!
//! This is the main entry point for the Core Deck companion application.
//! It initializes all modules and runs the main event loop.

use core_deck::{
    core::{
        config::Config,
        events::{AppEvent, EventSender, TrayAction},
        sessions::SessionId,
        state::AppState,
        tabs::{TabEntry, TabState},
    },
    daemon_client::DaemonClient,
    hid::{keycodes::{qmk_keycode_to_egui_key, qmk_keycode_to_terminal_bytes}, SoftKeyEditState},
    pty::{resolve_login_env, PtyWrapper},
    window::{TerminalAction, TerminalWindowState},
};
#[cfg(target_os = "macos")]
use core_deck::macos::{create_menu_bar, init_menu_sender, update_recent_sessions_menu, MenuAction};
use anyhow::Result;
...
struct App {
    ...
    /// Daemon client for device communication via WebSocket
...
impl App {
    ...
    fn start_claude_for_session(
        ...
        let (actual_resume_session, is_new_session, needs_session_id_resolution) = match &resume_session {
            ...
            Some(id) => {
                ...
                use core_deck::core::claude_sessions::session_exists;
    ...
    fn send_hid_for_active_session(&mut self) {
        if let Some(session) = self.terminal_window.session_manager.active_session() {
            ...
            let (task, task2) = match &session.current_task {
                Some(t) => {
                    let (line1, line2) = core_deck::core::text_compact::split_task_lines(t);
                    (Some(line1), line2)
                }
                None => (None, None),
            };
            ...
            if let Some(ref client) = self.daemon_client {
                if let Err(e) = client.send_display_update(&session_name, task.as_deref(), task2.as_deref(), &tabs, active) {
                    debug!("Failed to send display update: {}", e);
                }
                ...
                if let Err(e) = client.set_mode(mode) {
        ...
        if let Some(session) = self.terminal_window.session_manager.get_session_mut(session_id) {
            if session.hid_alert_active {
                ...
                if let Some(idx) = hid_tab_idx {
                    if let Some(ref client) = self.daemon_client {
                        if let Err(e) = client.clear_alert(idx) {
                            debug!("Failed to clear alert on input: {}", e);
                        }
                    }
                }
            }
    ...
    fn handle_terminal_action(&mut self, action: TerminalAction, event_loop: &ActiveEventLoop) {
        match action {
            ...
            TerminalAction::CloseTab(session_id) => {
                ...
                if let Some(ref client) = self.daemon_client {
                    for (_, old_idx, session_name, text, details) in &shifted_alerts {
                        ...
                        let _ = client.clear_alert(*old_idx);
                        // Re-send at the new index (old - 1)
                        let new_idx = old_idx - 1;
                        if let Some(ref text) = text {
                            let _ = client.send_alert(new_idx, session_name, text, details.as_deref());
                        }
                    }
                }
            ...
            TerminalAction::SwitchTab(session_id) => {
                ...
                if let Some(session) = self.terminal_window.session_manager.get_session_mut(session_id) {
                    ...
                    if session.hid_alert_active {
                        ...
                        if let Some(idx) = hid_tab_idx {
                            if let Some(ref client) = self.daemon_client {
                                if let Err(e) = client.clear_alert(idx) {
                                    debug!("Failed to clear alert on tab switch: {}", e);
                                }
                            }
                        }
                    }
                ...
                #[cfg(target_os = "macos")]
                if let Some(session) = self.terminal_window.session_manager.get_session(session_id) {
                    use core_deck::core::claude_sessions::get_sessions_for_directory;
            ...
            TerminalAction::ReadSoftKeys => {
                if let Some(ref client) = self.daemon_client {
                    ...
                    self.terminal_window
                        .set_soft_key_error("Daemon not connected".to_string());
                }
            }
            TerminalAction::ApplySoftKeys(keys) => {
                if let Some(ref client) = self.daemon_client {
                    for (i, key) in keys.iter().enumerate() {
                        ...
                        if let Err(e) = client.set_soft_key(i as u8, key_type, &data, true) {
            ...
            TerminalAction::HidDisplayUpdate { session, task, task2, tabs, active } => {
                if let Some(ref client) = self.daemon_client {
                    if let Err(e) = client.send_display_update(&session, task.as_deref(), task2.as_deref(), &tabs, active) {
                        debug!("Failed to send display update: {}", e);
                    }
                }
            }
            TerminalAction::HidAlert { tab, session, text, details } => {
                if let Some(ref client) = self.daemon_client {
                    if let Err(e) = client.send_alert(tab, &session, &text, details.as_deref()) {
            ...
            TerminalAction::HidClearAlert(tab) => {
                if let Some(ref client) = self.daemon_client {
                    if let Err(e) = client.clear_alert(tab) {
                        debug!("Failed to clear alert: {}", e);
                    }
                }
            }
    ...
    fn handle_event(&mut self, event: AppEvent, event_loop: &ActiveEventLoop) {
        match event {
            AppEvent::DaemonConnected => {
                info!("Daemon WebSocket connected");
                {
                    let mut state = self.state.write();
                    state.daemon_connected = true;
                }
                self.terminal_window.daemon_connected = true;
            }
            AppEvent::DaemonDisconnected => {
                info!("Daemon WebSocket disconnected");
                {
                    let mut state = self.state.write();
                ...
                self.terminal_window.daemon_connected = false;
                self.terminal_window.hid_connected = false;
                self.terminal_window.device_name = None;
                self.terminal_window.firmware_version = None;
                // Clear all HID alert flags since daemon is gone
            ...
            AppEvent::HidConnected { device_name, firmware_version } => {
                info!("HID device connected: {} (fw {})", device_name, firmware_version);
                {
                    let mut state = self.state.write();
                    state.hid_connected = true;
                    state.device_name = Some(device_name.clone());
                    state.firmware_version = Some(firmware_version.clone());
                }
                self.terminal_window.hid_connected = true;
                self.terminal_window.device_name = Some(device_name);
                self.terminal_window.firmware_version = Some(firmware_version);
            ...
            AppEvent::HidDisconnected => {
                ...
                self.terminal_window.device_name = None;
                self.terminal_window.firmware_version = None;
                self.terminal_window.device_popup_open = false;
            ...
            AppEvent::HidKeyEvent { keycode } => {
                ...
                if keycode == core_deck::hid::keycodes::QmkKeycode::F20 as u16 {
    ...
    fn handle_menu_action(&mut self, action: MenuAction, event_loop: &ActiveEventLoop) {
        ...
        match action {
            ...
            About => {
                // Show about dialog
                rfd::MessageDialog::new()
                    .set_title("About Core Deck")
                    .set_description("Core Deck v0.1.0\n\nA companion app for Claude Code CLI.")
            ...
            ReportIssue => {
                ...
                if let Err(e) = open::that("https://github.com/vden/coredeck/issues/new") {
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // Connect to daemon (starts background WS connection with auto-reconnect)
        let daemon_addr = coredeck_protocol::DEFAULT_DAEMON_ADDR;
        match DaemonClient::connect(daemon_addr, self.event_tx.clone()) {
            Ok(client) => {
                self.daemon_client = Some(client);
                info!("Daemon client started (connecting to {})", daemon_addr);
            }
            Err(e) => {
                error!("Failed to start daemon client: {}", e);
            }
        }
    }
...
fn setup_macos_app(event_tx: EventSender) {
    ...
    unsafe {
        ...
        let app_name = NSString::alloc(nil).init_str("Core Deck");
...
fn main() -> Result<()> {
    ...
    info!("Starting Core Deck companion app");
