...
/// Create the App menu (Core Deck)
#[allow(deprecated)]
unsafe fn create_app_menu() -> id {
    let menu: id = msg_send![class!(NSMenu), new];
    let title = NSString::alloc(nil).init_str("Core Deck");
    let _: () = msg_send![menu, setTitle: title];

    // About Core Deck
    let about = create_action_menu_item("About Core Deck", None, None, MenuAction::About);
    ...
    // Hide Core Deck (Cmd+H) - use standard action
    let hide_title = NSString::alloc(nil).init_str("Hide Core Deck");
    ...
    // Hide Core Deck (Cmd+Q) - hides window to tray, app keeps running
    ...
    // Quit Core Deck (Opt+Cmd+Q) - really quit the application
...
fn get_or_create_menu_handler() -> id {
    ...
    MENU_HANDLER.get_or_init(|| {
        unsafe {
            ...
            let class_name = "CoreDeckMenuHandler";
            ...
            let handler_class = if let Some(cls) = existing_class {
                ...
                let mut decl = ClassDecl::new(class_name, superclass)
                    .expect("Failed to create CoreDeckMenuHandler class");
...
fn get_or_create_context_menu_handler() -> id {
    ...
    CONTEXT_MENU_HANDLER.get_or_init(|| {
        unsafe {
            let class_name = "CoreDeckContextMenuHandler";
            let existing_class = Class::get(class_name);
            ...
            let handler_class = if let Some(cls) = existing_class {
                ...
                let superclass = class!(NSObject);
                let mut decl = ClassDecl::new(class_name, superclass)
                    .expect("Failed to create CoreDeckContextMenuHandler class");
