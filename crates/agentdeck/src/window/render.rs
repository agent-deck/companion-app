...
pub struct RenderParams<'a> {
    ...
    pub device_popup_open: &'a mut bool,
...
pub fn render_tab_bar(
    ...
    ui.horizontal(|ui| {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            // Connection indicator (daemon + device state)
            let daemon_connected = params.daemon_connected;
            let (icon_bytes, icon_uri, icon_tint) = if !daemon_connected {
                (
                    DECK_DISCONNECTED_SVG,
                    "bytes://deck-disconnected.svg",
                    color_scheme.secondary_foreground().linear_multiply(0.4), // Dim gray
                )
            ...
            if icon_response.clicked() {
                *params.device_popup_open = !*params.device_popup_open;
            }
...
/// Render the device info popup (daemon/device connection states)
pub fn render_device_popup(
    ctx: &egui::Context,
    device_popup_open: &mut bool,
    ...
    let popup_id = egui::Id::new("device_info_popup");
    let popup_width = if hid_connected { 240.0 } else { 260.0 };

    // Position: top-right area, below the tab bar
    let screen_rect = ctx.screen_rect();
    let popup_x = screen_rect.max.x - popup_width - 12.0;
    let popup_y = TAB_BAR_HEIGHT + 4.0;
    ...
    let area_response = egui::Area::new(popup_id)
        ...
        .show(ctx, |ui| {
            egui::Frame::popup(ui.style())
                .fill(color_scheme.tab_bar_background())
                .stroke(egui::Stroke::new(1.0, color_scheme.secondary_foreground().linear_multiply(0.3)))
                .rounding(8.0)
                .inner_margin(16.0)
                .shadow(egui::Shadow {
                ...
                .show(ui, |ui| {
                    ...
                    if !daemon_connected {
                        ...
                        ui.vertical_centered(|ui| {
                            ui.add(
                                egui::Image::from_bytes(
                                    "bytes://deck-disconnected-lg.svg",
                                    DECK_DISCONNECTED_SVG,
                                )
                                .fit_to_exact_size(egui::vec2(48.0, 48.0))
                                .tint(color_scheme.secondary_foreground().linear_multiply(0.4)),
                            );
                        });
                        ...
                        // --- Device connected (via daemon) ---
                        ui.vertical_centered(|ui| {
                            ui.add(
                                egui::Image::from_bytes(
                                    "bytes://deck-connected-lg.svg",
                                    DECK_CONNECTED_SVG,
                                )
                                .fit_to_exact_size(egui::vec2(48.0, 48.0))
                                .tint(egui::Color32::from_rgb(34, 139, 34)),
                            );
                        });
                        ...
                        ui.vertical_centered(|ui| {
                            ui.label(
                                egui::RichText::new(format!("Firmware: v{}", version))
                                    .size(12.0)
                                    .color(color_scheme.secondary_foreground()),
                            );
                        });
                        ...
                        ui.vertical_centered(|ui| {
                            ui.horizontal(|ui| {
                                ui.add_space((popup_width - 32.0 - 90.0) / 2.0);
                        ...
                        ui.vertical_centered(|ui| {
                            ui.add(
                                ...
                                .fit_to_exact_size(egui::vec2(48.0, 48.0))
                                .tint(egui::Color32::from_rgb(100, 60, 60)),
                            );
                        });
                        ...
                        ui.vertical_centered(|ui| {
                            ui.label(
                                egui::RichText::new("Get your Agent Deck at")
                                    .size(12.0)
                                    .color(color_scheme.secondary_foreground()),
                            );
                        });
                        ...
                        ui.vertical_centered(|ui| {
                            let link = ui.add(
                                egui::Button::new(
                                    egui::RichText::new("agentdeck.sh")
                                        .size(13.0)
                            ...
                            if link.clicked() {
                                super::input::open_url("https://agentdeck.sh");
                                *device_popup_open = false;
                            }
                        });
                    }
